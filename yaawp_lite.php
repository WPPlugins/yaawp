...
define('YAAWP_BASE', dirname(__FILE__));
define('YAAWP_BASE_DS', DIRECTORY_SEPARATOR);
...
if( ! class_exists( 'yaawp' ) ) {
  ...
  class yaawp {
    ...
    function __construct () {
      ...
      define('pluginUrl', $this->pluginUrl);
      ...
      add_filter( 'pre_get_posts', array( &$this, 'yaawp_pre_get_posts') );

      add_shortcode( 'yaawp_asin', array(&$this, 'yaawp_asin_shortcode') );

    }

    function yaawp_pre_get_posts ($wp_query) {
        if( isset( $wp_query->shop_category ) ) return;
        $wp_query->query_vars['posts_per_page'] = get_option('yaawp_product_number');
    }
    ...
    function yaawp_user_main () { 
      ...
      wp_register_style( 'yaawp-social-css', $this->pluginUrl.'assets/css/social.css', array(),'0.16','all');
      wp_enqueue_style( 'yaawp-social-css' );

      wp_register_style( 'yaawp-css', $this->pluginUrl.'assets/css/style.css', array(),'0.62','all');
      wp_enqueue_style( 'yaawp-css' );

      wp_enqueue_script( 'yaawp-js', $this->pluginUrl.'assets/js/frontend.js', array( 'jquery' ),'0.71');
      wp_localize_script( 'yaawp-js', 'yaawp',
        array(
          'ajaxurl'   => admin_url( 'admin-ajax.php' ),
          'nonce'     => wp_create_nonce( 'yaawp-nonce' ),
          'pluginUrl' => plugins_url() . '/yaawp/'
        )
      );
    ...
    function yaawp_admin_main () {
      ...
      wp_enqueue_style( 'yaawp-social-css', $this->pluginUrl.'assets/css/social.css', array(),'0.16','all');
      wp_enqueue_style( 'yaawp-css', $this->pluginUrl.'assets/css/admin.css', array(),'0.63','all');
      wp_enqueue_script( 'yaawp-js', $this->pluginUrl.'assets/js/admin.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ),'0.69');
      wp_localize_script( 'yaawp-js', 'yaawp',
        array(
          'ajaxurl'             => admin_url( 'admin-ajax.php' ),
          'nonce'               => wp_create_nonce( 'yaawp-nonce' ),
    ...
    function getProductLayout () {
      ...
      $layout = get_option('yaawp_product_layout');

      if ( !$layout ) {
        add_option( 'yaawp_product_layout', 'grid', '', 'yes' );
        $layout = get_option('yaawp_product_layout');
      }

      switch($layout) {
        case 'list':
          $options = '<option value="list" selected>'.__('Liste', $this->var_sTextdomain).'</option>';
          $options .= '<option value="grid">'.__('Grid', $this->var_sTextdomain).'</option>';
        break;
        case 'grid':
          $options = '<option value="list">'.__('Liste', $this->var_sTextdomain).'</option>';
          $options .= '<option value="grid" selected>'.__('Grid', $this->var_sTextdomain).'</option>';
        break;
      }
    ...
    function getTemplate( $template = false ) {
      ...
      foreach (new DirectoryIterator($this->pluginPath.'templates/frontend/product') as $fileInfo) {
        ...
        $data = get_plugin_data( plugin_dir_path( __FILE__ ) . 'templates/frontend/product/' . $fileInfo->getFilename() );
        $templates .= '<option value="'.$fileInfo->getFilename().'"'.$value.'>'.$data['Name'].'</option>';
    ...
    function yaawp_taxonomy_column_shop ( $column, $post_id ) {
      ...
      if ( $column == 'categorie') {
        ...
        if ( empty( $terms ) ) {
           ...
           $out[] = '<a style="color:red;">'.__('Kategorie wurde gel&ouml;scht.', $this->var_sTextdomain).'</a>';
          ...
          if ( is_array($terms) ) {
            ...
            $out[] = '<a href="' .get_term_link($terms->slug, 'shop_category') .'">'.$terms->name.'</a>';
          }
    ...
    function Backend () {
      ...
      switch ($_POST['method']) {
        ...
        case 'UpdateSocial':
          ...
          if ( update_option('yaawp_product_social', $_POST) ) {

            $response = json_encode(array(
              'status' => 1,
              'msg' => __('Einstellungen wurden aktualisiert.', $this->var_sTextdomain)
            ));
            ...
            $response = json_encode(array(
              'status' => 0,
              'msg' => __('Einstellungen wurden nicht aktualisiert.', $this->var_sTextdomain)
            ));
        ...
        case 'UpdateProduct':
          ...
          foreach ($post_results as $post) {
            if ( ! update_post_meta( $post->ID, 'yaawp_template', get_option('yaawp_product_template') ) ) $error = 1;
          }
          ...
          if ( $error == 0 ) {
            ...
            $response = json_encode(array(
              'status' => 1,
              'msg' => __('Standard Produkt Layout (Einzelseite) aktualisiert.', $this->var_sTextdomain)
            ));
            ...
            $response = json_encode(array(
              'status' => 0,
              'msg' => __('Standard Produkt Layout (Einzelseite) nicht aktualisiert.', $this->var_sTextdomain)
            ));
        ...
        case 'UpdateCat':
          ...
          $option_name = 'yaawp_product_cat';
          $option_value = $_POST['orders'];
          ...
          if ( get_option( $option_name ) != $option_value ) {
            ...
            if ( add_option( $option_name, $option_value, '', 'yes' ) ) {
        ...
        case 'UpdateProductTemplate':
          ...
          if ( update_option('yaawp_product_template', $_POST['yaawp_product_template']) ) {

            $response = json_encode(array(
              'status' => 1,
              'msg' => __('Standard Produkt Layout wurde aktualisiert.', $this->var_sTextdomain)
            ));
            ...
            $response = json_encode(array(
              'status' => 0,
              'msg' => __('Standard Produkt Layout wurde nicht aktualisiert.', $this->var_sTextdomain)
            ));
        ...
        case 'UpdateProductNumber':

          if ( update_option('yaawp_product_number', $_POST['yaawp_product_number']) ) {

            $response = json_encode(array(
              'status' => 1,
              'msg' => __('Anzahl der Produkte pro Seite wurde aktualisiert.', $this->var_sTextdomain)
            ));
            ...
            $response = json_encode(array(
              'status' => 0,
              'msg' => __('Anzahl der Produkte pro Seite wurde nicht aktualisiert.', $this->var_sTextdomain)
            ));
        ...
        case 'UpdateProductOption':
          ...
          if ( get_option( 'yaawp_product_title' ) != $_POST['yaawp_product_title'] || get_option( 'yaawp_product_exc' ) != $_POST['yaawp_product_exc']  ) {

            if ( update_option( 'yaawp_product_title' , $_POST['yaawp_product_title']) || update_option( 'yaawp_product_exc' , $_POST['yaawp_product_exc']) ) {
            ...
            if ( add_option( 'yaawp_product_title', $_POST['yaawp_product_title'], '', 'no' ) || add_option( 'yaawp_product_exc', $_POST['yaawp_product_exc'], '', 'no' )) {
        ...
        case 'UpdateProductLayout':

          if ( update_option('yaawp_product_layout', $_POST['yaawp_product_layout']) ) {
    ...
    function yaawp_settings_page () {
      ...
      $tabs = array(
        ...
        'shop' => __('Shop', $this->var_sTextdomain),
        'social' => __('Social', $this->var_sTextdomain),
        'cat' => __('Kategorien', $this->var_sTextdomain)
      );
    ...
    function yaawp_template_redirect () {
      ...
      if ( preg_match("/(shop)\//is", $_SERVER['REQUEST_URI']) ) {

        if ( substr($_SERVER['REQUEST_URI'],-6) === '/shop/' ) {
    ...
    function yaawp_template_include( $template_path ) {
      ...
      if ( get_post_type() == 'shop' ) { 
        ...
        if ( is_single() ) {
          ...
          if ( !$template ) $template = 'product.php'; // fallback
    ...
    private function _yaawp_build_placholder ( $_item ) {
      ...
      $this->Placeholder = array(
        ...
        'Rating',
        'Tags',
        'Availability'                  => (string) ($_item->Offers->Offer->OfferListing->AvailabilityAttributes->AvailabilityType == 'now') ? __( 'verf&uuml;gbar', $this->var_sTextdomain) : __( 'nicht verf&uuml;gbar', $this->var_sTextdomain),
    ...
    private function _yaawp_renew_data ( $asin, $post_id ) {
      ...
      $ya = new yaawp_amazon;
      $response = $ya->ItemLookup($asin); sleep(1);
    ...
    function yaawp_asin_shortcode ( $atts ) {
      ...
      if ( strlen($title) > $trim ) $title = substr($title,0,$trim) . ' ...';

      if ( strlen($ctitle) > 0 ) {
        $this->Placeholder['Title'] = $ctitle;
        $title = $ctitle;
      }
      ...
      switch($template) {
        case 1:
          ...
          $html .= '<a href="' . $this->Placeholder['DetailPageURL'] . '" title="' .  $this->Placeholder['Title'] . '" target="_blank" rel="external nofollow">' .  $title . '</a><br />';
          if (!empty($this->Placeholder['Binding'])) {
        ...
        break;
        case 4:
          $html .= '<a href="' . $this->Placeholder['DetailPageURL'] . '" title="' . $this->Placeholder['Title'] . '" target="_blank" rel="external nofollow">' . $title . '</a>';
        break;
      }
    ...
    function yaawp_import ( $atts ) {
      ...
      // $response = $ya->ItemSearch($suchwort,$kategorie,$page,'Large,Images,EditorialReview,ItemAttributes,Reviews,OfferSummary,Offers',$noteid);
      ...
      foreach ($response->Items->Item as $mk => $_item) {
        ...
        if ( isset($this->Placeholder['ProductDescription']) && strlen($this->Placeholder['ProductDescription']) > 200 ) {
          $_EditorialReview_1 = substr($this->Placeholder['ProductDescription'], 0, 199).'...';
          $_EditorialReview_2 = substr($this->Placeholder['ProductDescription'], 199);
          $this->Placeholder['ProductDescription'] = $_EditorialReview_1;
        } elseif ( isset($this->Placeholder['ProductDescription']) && strlen($this->Placeholder['ProductDescription']) < 200 ) {
          $_EditorialReview_1 = $this->Placeholder['ProductDescription'];
        } else {
          $_EditorialReview_1 = '...';
        }
    ...
    function yaawp_import_data ( $atts ) {
      ...
      if ( strlen($this->Placeholder['ProductDescription']) > 100 ) {
        $this->Placeholder['ProductDescription'] = wp_strip_all_tags($this->Placeholder['ProductDescription'],true);
        $this->Placeholder['ProductDescription'] = substr($this->Placeholder['ProductDescription'],0, 99).' ...';
      } else {
        $this->Placeholder['ProductDescription'] = wp_strip_all_tags($this->Placeholder['ProductDescription'],true);
        $this->Placeholder['ProductDescription'] = $this->Placeholder['ProductDescription'].' ...';
      }
      ...
      if ( !$postID ) {
        ...
        $ImageSets = NULL;
        if ( isset($_item->ImageSets) ) {
          $ImageSets = yaawp_objectsIntoArray($_item->ImageSets);
        }
        ...
        $Features = null;
        if (isset($_item->ItemAttributes->Feature)) {
          foreach ($_item->ItemAttributes->Feature as $f) {
        ...
        add_post_meta($postID, 'Features', $Features , true); // New
        ...
        add_post_meta($postID, 'EAN', $this->Placeholder['EAN'] , true);
        add_post_meta($postID, 'LastCheck', date('Y-m-d H:i:s') , true);

        $yaawp_product_template  = get_option('yaawp_product_template');

        if ( $yaawp_product_template != '' ) {
          add_post_meta($postID, 'yaawp_template', $yaawp_product_template , true);
        }
